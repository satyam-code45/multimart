  ...
  getMany: baseProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      if (input.tenantSlug) {
        ...
        //it means if we are not on the tenant storefront we are on public storefront in that case we hide the private product
        where["isPrivate"] = {
          not_equals: true,
        };
      }
      ...
      const data = await ctx.db.find({
        ...
        //depth: 0 will only return the category id so depth1 as it will return the entire product
        where,
        sort,
        page: input.cursor,
        limit: input.limit,
